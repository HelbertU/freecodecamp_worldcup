...
echo "$($PSQL "(SELECT name as namet FROM games INNER JOIN teams ON games.winner_id=teams.team_id WHERE year=2014 AND round='Eighth-Final') UNION (SELECT name as namet FROM games INNER JOIN teams ON games.opponent_id=teams.team_id WHERE year=2014 AND round='Eighth-Final') ORDER BY namet;")"
